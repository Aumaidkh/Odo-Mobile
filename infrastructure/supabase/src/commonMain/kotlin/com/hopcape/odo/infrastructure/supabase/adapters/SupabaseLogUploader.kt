 ...
 *
 * [owners] resolves the storage path's owner segment. `OwnerId.LOCAL_PLACEHOLDER` (nobody
 * signed in yet) answers [LogUploadResult.RETRY] without attempting anything — D4: pre-auth
 * files are held, not dropped, and age out on their own via retention if nobody ever signs in.
 */
internal class SupabaseLogUploader(
    ...
    /**
     * One id for this uploader's lifetime — effectively one per process, not persisted across
     * restarts. `SecureStore` was considered and rejected for this: its own doc says it exists
     * for exactly one thing (the Supabase session) and is deliberately slow (Keystore-backed);
     * a device id is not a secret. A per-restart id still groups every row from one cold start
     * together — most of this column's triage value — real cross-restart stability is a
     * follow-up (§7.3).
     */
    ...
    override suspend fun upload(file: LogFileHandle, bytes: ByteArray): LogUploadResult {
        ...
        if (ownerId == OwnerId.LOCAL_PLACEHOLDER) return LogUploadResult.RETRY

        return telemetry.span(operation = SupabaseTelemetry.UPLOAD, resource = BUCKET) {
            uploadObjectThenIndexRow(ownerId, file, bytes)
        }
    }
    ...
    private suspend fun uploadObjectThenIndexRow(
        ...
        val path = "${ownerId.value}/$deviceId/${file.name}"
        ...
        if (!putStatus.isSuccess()) {
            ...
            return if (putStatus.value in CLIENT_ERROR_RANGE) LogUploadResult.REJECTED else LogUploadResult.RETRY
        }

        insertIndexRow(path, file)
        return LogUploadResult.DELIVERED
    }

    /** Best-effort: the object already landed, so a failure here is recorded, not retried. */
    private suspend fun insertIndexRow(path: String, file: LogFileHandle) {
        runCatching {
            postgrest.upsert(
                table = LOG_UPLOADS_TABLE,
                serializer = LogUploadRow.serializer(),
                rows = listOf(LogUploadRow.from(path, deviceId, appInfo.versionName, file)),
                returnRows = false,
            )
...
private data class LogUploadRow(
    ...
    companion object {
        fun from(path: String, deviceId: String, appVersion: String, file: LogFileHandle) = LogUploadRow(
            deviceId = deviceId,
            storagePath = path,
