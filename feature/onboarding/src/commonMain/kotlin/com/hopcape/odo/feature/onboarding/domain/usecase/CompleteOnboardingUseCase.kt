 ...
 *
 * **This is where "onboarding needs a name and a goal" lives** — it is onboarding's rule,
 * not a universal truth about owners. The server creates a profile row at signup with
 * neither, so [OwnerProfile] itself accepts their absence; only this flow refuses to finish
 * without them. Keeping the rule here is what lets the shared kernel stay free of any one
 * feature's policy.
 *
...
internal class CompleteOnboardingUseCase(
    ...
    suspend operator fun invoke(
        ...
        zipOrAccumulate(
            { OwnerName.of(command.name).bind() },
            { ensureNotNull(command.goal) { DomainError.MissingOnboardingGoal } },
        ) { validName, validGoal ->
            OwnerProfile.new(
                id = currentOwner.currentOwnerId(),
                name = validName,
                goal = validGoal,
