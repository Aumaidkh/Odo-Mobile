...
class DocumentMappersTest {
    ...
    fun toDomain_handlesNullOptionals() {
        val document = row(title = null, issuedDate = null, expiryDate = null).toDomain()
    ...
    fun toDomain_readsUnknownTypeAsOther() {
        ...
        assertEquals(DocumentType.OTHER, row(docType = "HOLOGRAM").toDomain().type)
    }

    @Test
    fun toDomain_readsUnknownSourceAsUploaded() {
        val document = row(docSource = "TELEPORTED").toDomain()
