...
import com.hopcape.odo.core.data.sync.noopBlobUploaderFileStore
import com.hopcape.odo.core.data.sync.silentDataTelemetry
...
class ServiceLogBillIdSyncTest {
    ...
    private fun table(): Pair<ServiceLogSyncTable, com.hopcape.odo.core.data.db.OdoDatabase> {
        val (database, _) = inMemoryDatabase()
        val syncTable = ServiceLogSyncTable(
            database = database,
            remote = FakeServiceLogRemoteDataSource(),
            blobs = BlobUploader(noopBlobUploaderFileStore, FakeRemoteFileStorage(), silentDataTelemetry()),
            carId = { CAR },
        )
        return syncTable to database
    }

    private fun com.hopcape.odo.core.data.db.OdoDatabase.insertScannedLog() =
