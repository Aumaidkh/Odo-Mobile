...
interface FuelFillRemoteDataSource {
    ...
    /** Fills changed since [since] (null = never synced, so everything). */
    suspend fun fetchSince(carId: String, since: Instant?): List<FuelFillDto>
...
internal class FakeFuelFillRemoteDataSource : FuelFillRemoteDataSource {
    override suspend fun fetchSince(carId: String, since: Instant?): List<FuelFillDto> = emptyList()
