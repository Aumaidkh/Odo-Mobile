...
import com.hopcape.crashreporting.internal.destinations.CrashlyticsDestination
...
// APM PerformanceFactory. The ONE place that names concrete types
// (the vendor destinations, the redactor, the platform file store);
// everything else depends only on interfaces (DIP). Internal: the
// public entry points are the CrashReporter facade and the
// crashReportingModule Koin binding, both of which feed a CrashConfig
// through here.
// ─────────────────────────────────────────────────────────────
internal object CrashReporterFactory {
    ...
    fun create(config: CrashConfig): CrashRecorder {
        ...
        val rawDestinations = buildList {
            add(CrashlyticsDestination())                 // primary vendor
