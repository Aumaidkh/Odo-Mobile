...
- **The local DB has no migrations yet.** There are no `.sqm` files, so SQLDelight's schema version never moves and a new `.sq` table is created on a **fresh install only**. An app that is already installed keeps its old database and every query against the new table fails with `no such table` — which the repository reports as a non-fatal and an empty screen, not a crash. So when a slice adds a table, reinstall or clear app data before testing it, and say so when handing the work over. This is only acceptable while nothing is released; real migrations land before launch (DB_SCHEMA §15 is the ordering).
