...
| 25 | `log_uploads` | MVP | Index over uploaded diagnostic log files (§9.20) | yes (insert + read only) |
...
| `app-logs` | private | Gzipped diagnostic log files | `{owner_id}/{device_id}/{timestamp}.log.gz` |
...
Capped at 1000 rows / 7 days so a permanently-failing destination cannot grow it forever —
...
### 9.20 Observability: `log_uploads`

Not a domain table — an index over the `app-logs` Storage bucket (§7), one row per uploaded
diagnostic log file. Log lines themselves never enter Postgres; a session can be thousands of
lines, and the operational database has no business carrying that. This is what makes the
bucket triageable in SQL — "who has a file with a fatal in it", "did `error_count` spike on
1.4.0" — without opening a single object (see `docs/LOGGING_PLAN.md` §7.3 for the client side).
...
CREATE TABLE log_uploads (
    ...
    storage_path  text NOT NULL UNIQUE,        -- the app-logs object this row describes
    opened_at     timestamptz NOT NULL,        -- from the file name
    sealed_at     timestamptz NOT NULL,
    uploaded_at   timestamptz NOT NULL DEFAULT now(),
    size_bytes    bigint NOT NULL,             -- compressed
    app_version   text NOT NULL,
    os_version    text,                        -- not populated by the client yet
    -- Nullable on purpose: a file recovered from a process that died before it could seal
    -- has no live counters. NULL means "unknown" and must never be read as "zero errors".
    line_count    integer,
    warn_count    integer,
    error_count   integer,
    had_fatal     boolean
...
CREATE INDEX idx_log_uploads_owner ON log_uploads (owner_id, uploaded_at DESC);
CREATE INDEX idx_log_uploads_app_version ON log_uploads (app_version, uploaded_at DESC);
CREATE INDEX idx_log_uploads_fatal ON log_uploads (uploaded_at DESC) WHERE had_fatal;
```

`owner_id` is stamped by `stamp_owner_from_auth()` (§10.2a) — not the car-derived trigger every
other table uses, since this table has no `car_id` to derive it from.

Deliberately outside the sync engine and carries none of the mandatory sync columns
(SYNC_DESIGN §4.1): it has no local counterpart, and is write-once server-side telemetry, not
user content. No soft delete either — `deleted_at` exists so user content survives a mistake;
this hard-deletes with the account (§13), and a `pg_cron` job separately prunes rows whose
object the bucket's 30-day lifecycle rule has already removed.

**Insert ordering is load-bearing.** The client inserts a row only after the Storage `PUT`
answers 2xx — never before — so a row here always names an object that actually exists. If the
object lands and the row insert then fails, the client does not retry: re-uploading identical
bytes for a row that only failed to record itself would just duplicate storage, so it deletes
its local copy and moves on. A small, accepted gap: a delivered file with no index row.
...
### 10.2a Stamp `owner_id` from the session (no parent row)

Every table above has a `car_id` to derive `owner_id` from. `log_uploads` (§9.20) is the first
that doesn't — it belongs to the signed-in user directly, with no car in between — so instead
of joining out to find the owner, this reads `auth.uid()` straight from the request's JWT. The
client's `LogUploadRow` payload never includes `owner_id` at all (docs/LOGGING_PLAN.md §7.3),
so there is nothing here to overwrite; this just makes it unforgeable if a client ever did send one.

```sql
CREATE OR REPLACE FUNCTION stamp_owner_from_auth()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
...
CREATE TRIGGER trg_log_uploads_owner
    BEFORE INSERT ON log_uploads
    FOR EACH ROW EXECUTE FUNCTION stamp_owner_from_auth();
```
...
-- log_uploads (§9.20): the client both writes (after a successful Storage upload) and reads
-- its own rows for "send diagnostics" triage, but never updates or deletes one — a row is a
-- fact about an object that was uploaded, not something that changes after the fact.
ALTER TABLE log_uploads ENABLE ROW LEVEL SECURITY;
CREATE POLICY insert_own_log_upload ON log_uploads
    FOR INSERT TO authenticated WITH CHECK (owner_id = (SELECT auth.uid()));
CREATE POLICY read_own_log_uploads ON log_uploads
    FOR SELECT TO authenticated USING (owner_id = (SELECT auth.uid()));
...
| Diagnostic logs (`app-logs` Storage + `log_uploads`, §9.20) | 30 days | Object + index row hard-deleted (CASCADE); also pruned on the 30-day schedule regardless of account status |
...
3. `DELETE FROM profiles WHERE id = :owner` (CASCADE clears the rest, including `log_uploads`).
4. `auth.admin.deleteUser(:owner)`.

**`app-logs` also has its own independent 30-day lifecycle**, separate from account deletion:
a Storage lifecycle rule expires objects after 30 days, and a `pg_cron` job prunes `log_uploads`
rows whose object it already removed. Diagnostic logs are the one thing in this schema retained
for weeks, not the life of the account — see `docs/LOGGING_PLAN.md` §7.3 for why (they exist to
debug a recent bug, not to be a record).
...
0024_log_uploads.sql           -- log_uploads table, stamp_owner_from_auth, RLS, app-logs bucket
-- Phase 2
