package com.hopcape.odo.web.admin.routing

import androidx.compose.runtime.getValue
import androidx.compose.runtime.mutableStateOf
import androidx.compose.runtime.setValue
import kotlinx.browser.window

/**
 * The [Router] the deployed app runs on: the address bar is the state.
 *
 * Every section is a real address, so a reload lands where you were and a link to
 * one is worth pasting into a message. Navigating therefore does two things in one
 * place — it pushes the address and it moves the app — and `popstate` moves the app
 * when somebody uses the back button.
 *
 * Refresh only survives because `landing/firebase.json` rewrites everything under
 * `/admin/` to this app's own `index.html`. Without that rewrite every section is a
 * 404 on reload, and nothing in this class can tell.
 */
class BrowserRouter : Router {

    /**
     * Read once. It cannot change without the document reloading, and reading it
     * per navigation would let a pushed URL redefine where the app thinks it is.
     */
    private val base: String = basePathOf(window.location.pathname)

    override var current: AdminRoute by mutableStateOf(read())
        private set

    init {
        // The back and forward buttons. The browser has already changed the URL by
        // the time this fires, so the address is again the thing to read.
        window.addEventListener("popstate") { current = read() }
    }

    override fun go(route: AdminRoute) {
        window.history.pushState(null, "", base + route.location())
        current = route
    }

    override fun replace(route: AdminRoute) {
        window.history.replaceState(null, "", base + route.location())
        current = route
    }

    /** The route the address bar currently describes. */
    private fun read(): AdminRoute = routeOf(window.location.pathname.removePrefix(base))
}
