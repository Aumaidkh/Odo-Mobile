...
object AdminIcons {
    ...
    /*
     * The social section's six tabs.
     *
     * Fetched from bootstrap-icons v1.11.3 rather than typed, like everything above: a path is
     * a few hundred characters of numbers and one wrong digit is a shape nobody can debug by
     * reading. These are the only glyphs the tab chips draw — the section first shipped with
     * "▸", "●" and "○" in the labels, and this panel carries no icon font, so all three
     * rendered as missing characters.
     */
    /** `gear` — the pipeline's own knobs. */
    val Settings = BootstrapIcon(
        listOf(
            IconPath(
    ...
    /** `calendar-week` — when it posts. */
    val Schedule = BootstrapIcon(
        listOf(
            IconPath(
    ...
    /** `person-badge` — the connected networks. */
    val Accounts = BootstrapIcon(
        listOf(
            IconPath(
    ...
    /** `telegram` — who is told, and who may approve. */
    val Telegram = BootstrapIcon(
        listOf(
            IconPath(
    ...
    /** `list-check` — what is waiting. */
    val Queue = BootstrapIcon(
        listOf(
            IconPath(
    ...
    /** `lightbulb` — the verified facts copy is written around. */
    val FactBank = BootstrapIcon(
        listOf(
            IconPath(
